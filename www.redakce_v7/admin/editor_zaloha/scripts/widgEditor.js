...
{
	if (widgAutoClean || confirm("chcete vymazat zdrojové kodovaní na pozadí pøenášeného textu?"))
	{
		var matchedHead = "";
		var matchedTail = "";
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "hyperlink":
				this.addButton(this.theList.id + "ButtonLink", "widgButtonLink", "vložit odkaz", "link");
				
				break;
				
			case "unorderedlist":
				this.addButton(this.theList.id + "ButtonUnordered", "widgButtonUnordered", "odrážkový seznam", "insertunorderedlist");
				
				break;
				
			case "orderedlist":
				this.addButton(this.theList.id + "ButtonOrdered", "widgButtonOrdered", " èíselný seznam", "insertorderedlist");
			...
			case "htmlsource":
				this.addButton(this.theList.id + "ButtonHTML", "widgButtonHTML", "HTML kód", "html");
				
				break;
			
			/*zaslepena moznost vyberu pisma P H1 H2 ....*/
			/*case "blockformat":
		...
		case "link":
			...
			else
			{
				if (theIframe.contentWindow.document.selection)
				{
					...
					if (theSelection == "")
					{
						alert("oznaète text který bude odkazovat na zadanou adresu.");
				...
				var theURL = prompt("vložte URL adresu pro odkaz:", "http://");
